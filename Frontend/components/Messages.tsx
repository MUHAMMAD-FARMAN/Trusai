...
const Messages = forwardRef<ComponentRef<typeof motion.div>, Record<never, never>>(function Messages(_, ref) {
  ...
  const { setSpeechEmotions } = useEmotionStore()

  useEffect(() => {
    if (messages.length > 0) {
      const lastMessage = messages[messages.length - 1]
      if (lastMessage.type === "user_message" && lastMessage.models.prosody?.scores) {
        setSpeechEmotions({ ...lastMessage.models.prosody.scores })
      }
    }
  }, [messages, setSpeechEmotions])
